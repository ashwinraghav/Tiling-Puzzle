...
public class PartIdentifier
{
			...
			int numCols) 
	{
		...
		Domino[][] correspondingMatrix = new Domino[yDim][xDim];
		
		for(int lcv1=startCell.getI() , iIndex = 0; iIndex<yDim; ++lcv1, ++iIndex)
			for (int lcv2= startCell.getJ(), jIndex = 0; jIndex<xDim; ++lcv2, ++jIndex)
				correspondingMatrix[iIndex][jIndex] = matrix[lcv1][lcv2];
					
		return new Tile(startCell, endCell, numDominos, dominoList, correspondingMatrix);
